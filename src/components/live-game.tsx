...
export function LiveGame({ game, allPlayers }: { game: Game; allPlayers: { id: string; name: string }[] }) {
  ...
  // Players whose final chips are recorded: a buy-in for them afterwards is almost always a slip.
  const finishedIds = new Set(game.bankOperations.filter((row) => !row.voidedAt && row.kind === "CASH_OUT").map((row) => row.playerId));
  ...
  return <div className="live-bank-page"><LiveRefresh />
    ...
    {open ? <GameBankForms context={context} players={seated} defaultBuyIn={game.defaultBuyIn} recent={recentTakes} finished={[...finishedIds]} /> : <div className="bank-notice"><h2>Time to settle up.</h2><p>All chips are returned. Final settlement below offsets each player’s buy-ins against their returns and records only the cash difference.</p></div>}
    ...
    <div className="bank-player-list">{game.entries.map((entry) => {
      ...
      return <article className="bank-player-card" key={entry.id}><header><Avatar name={entry.player.name} /><div><h3>{entry.player.name}</h3>{open ? null : <p>Buy-ins {fmtMoney(player.buyIn)} · chips {fmtMoney(player.cashOut)}</p>}</div></header>
        {open
          ? <><div className="player-bank-balances"><div><small>Bought in</small><strong>{fmtMoney(player.buyIn)}</strong></div><div><small>{finishedIds.has(entry.playerId) ? "Lent + final chips" : "Lent to others"}</small><strong className={player.cashOut ? "text-[var(--violet)]" : ""}>{fmtMoney(player.cashOut)}</strong></div></div>
            <p className={`running-result ${player.cashOut - player.buyIn > 0 ? "running-up" : player.cashOut - player.buyIn < 0 ? "running-down" : ""}`}>{finishedIds.has(entry.playerId)
              ? <>Result: <strong>{fmtMoney(player.cashOut - player.buyIn, { sign: true })}</strong></>
              : player.cashOut - player.buyIn > 0
                ? <>Already <strong>{fmtMoney(player.cashOut - player.buyIn)} up</strong>, plus the chips in front of them</>
                : player.cashOut - player.buyIn < 0
                  ? <>In for <strong>{fmtMoney(player.buyIn - player.cashOut)}</strong>, minus the chips in front of them</>
                  : <>Even so far, plus the chips in front of them</>}</p></>
